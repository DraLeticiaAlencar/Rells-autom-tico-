...
async function gerarAudio(texto, s) {
  if (!s.eleven_key || !s.eleven_voice_id) throw new Error("ElevenLabs não configurado.");
  adicionarHistorico({ status: "Gerando Audio", details: "Sintetizando voz com ElevenLabs..." });
  const audioPath = `./videos/audio_${Date.now()}.mp3`;
  const r = await axios.post(
    ...
    { headers: { "xi-api-key": s.eleven_key, "Content-Type": "application/json" }, responseType: "arraybuffer", timeout: 30000 }
  );
  fs.writeFileSync(audioPath, Buffer.from(r.data));
  adicionarHistorico({ status: "Audio Pronto", details: "Voz gerada com ElevenLabs." });
  return audioPath;
}
